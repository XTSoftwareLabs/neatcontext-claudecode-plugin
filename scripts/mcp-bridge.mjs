...
import { readDiscovery, request } from "./companion-client.mjs";
...
function neatContextSource() {
  ...
  return {
    async version() {
      ...
      try {
        const response = await request(discovery, "GET", "/v1/connection", { timeoutMs: 2500 });
        return response.status === 200 ? (response.json?.version ?? null) : null;
      } catch {
        return null;
      }
    },
...
async function handleMessage(message) {
  ...
  if (message.method === "initialize" && response && response.result) {
    ...
    started = true;
    lastVersion = await source.version();
    startVersionWatch();
  }
