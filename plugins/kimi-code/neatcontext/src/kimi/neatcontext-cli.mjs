// Command-line entry the Kimi Code skills call. Prints human-readable text that
...
async function commandUse(state, query) {
  ...
  print(
    `Could not connect "${target.name}". Make sure NeatContext desktop is open, ` +
  ...
  if (wanted === "auto") {
    print(
      "In auto mode this session switches context on its own, and tells you when it does. " +
        "Other Kimi Code sessions keep theirs."
    );
  }
}
...
// The model in the active Kimi Code session writes the capture spec: it is
